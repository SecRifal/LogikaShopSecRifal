...
function getCardHTML(product) {
    ...
    return `
        <div class="g1">
            <div class="good_text">
                <p class="good_name">${product.name}</p>
                <p class="good_price">${product.price}$</p>
            </div>
            <img class="good_img" src="imgs/${product.img}" alt="">
            
            <button class="good_buy"  data-product='${productData}'>Добавити у корзину.</button>
        </div>
    `;
}
...
getProducts().then(function (products) {
    let productsList = document.querySelector('.goods')
    if (productsList) {
    ...
    let buyButtons = document.querySelectorAll('.good_buy');
